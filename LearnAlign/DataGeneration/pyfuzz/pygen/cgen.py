...
class Declaration(object):
    ...
    def __init__(self, target, type='int'):
        self.target = target
        self.type = type
		

class MultipleDeclaration(object):
...
class CodeGenerator(object):
    ...
    def visit_block(self, depth, block, addCurlyBrackets=True):
        if len(block) == 0:
        ...
        if addCurlyBrackets:
            content.insert(0, self.code(depth, '{'))
    ...
    def visit(self, depth, node):
        ...
        line = "for (%s = 0; %s++; %s<%s)" % (node.pointer, node.pointer, node.pointer, iterable)
    ...
    def visit(self, depth, node):
        content = []
        content.append(self.code(depth, ["if (%s)" % node.clause]))
    ...
    def visit(self, depth, node):
        if node.args:
            args = ", ".join(["int %s" % arg for arg in node.args])
        else:
            args = ""

        fun = "".join(['int ', node.name, '(', args, ')'])
    ...
    def visit(self, depth, node):
        ...
        if(len(node.expression) > 0):
            code += map(
                lambda x: x.strip(),
                self.visit_block(0, node.expression, addCurlyBrackets=False))
        code = " ".join(code)
        code += ';'
        return [self.code(depth, code)]

    @visit.when(Declaration)
    def visit(self, depth, node):
        code = [node.type, node.target]
        code = " ".join(code)
        code += ';'
        return [self.code(depth, code)]
			
    @visit.when(MultipleDeclaration)
    def visit(self, depth, node):
        code = ", ".join(node.target)
        code = "%s %s = 0;" % (node.type, code)
        return [self.code(depth, code)]
